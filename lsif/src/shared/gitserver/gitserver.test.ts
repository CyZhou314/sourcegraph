...
describe('getDirectoryChildren', () => {
    it('should parse responses from gitserver', async () => {
        nock('http://frontend')
            .post('/.internal/git/42/batch-exec', {
                requests: [
                    { args: ['ls-tree', '--name-only', 'c', '--', 'foo/bar/baz/'] },
                    { args: ['ls-tree', '--name-only', 'c'] },
                ],
            })
            .reply(
                200,
                `[
                    {"stdout": "foo/bar/baz/a\\nfoo/bar/baz/b\\nfoo/bar/baz/c\\nfoo/bar/baz/d\\nfoo/bar/baz/e\\n"},
                    {"stdout": "a\\nb\\nc\\nd\\ne\\n"}
                ]`
            )
        ...
        expect(
            ...
            new Set(['foo/bar/baz/a', 'foo/bar/baz/b', 'foo/bar/baz/c', 'foo/bar/baz/d', 'foo/bar/baz/e']),
            new Set(['a', 'b', 'c', 'd', 'e']),
        ])
    })
})
